    ...
    Column(modifier = Modifier.fillMaxSize().padding(8.dp)) {
        ...
        LazyColumn(modifier = Modifier.weight(1f)) {
            ...
            if (uiState.reminders.isNotEmpty()) {
                ...
                items(uiState.reminders, key = { "rem-${it.id}" }) { note ->
                    ReminderRow(note = note, onClick = { onNoteSelected(note.id) })
                }
...
@Composable
private fun ReminderRow(note: Note, onClick: () -> Unit) {
    ...
    Row(
        ...
        Column {
            Text(note.title.ifBlank { "Без названия" }, style = MaterialTheme.typography.bodyMedium)
            note.reminderAt?.let {
                Text(
                    fmt.format(Date(it)),
